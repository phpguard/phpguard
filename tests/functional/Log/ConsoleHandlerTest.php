...
class ConsoleHandlerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $handler = new ConsoleHandler();
        $this->assertInstanceOf('PhpGuard\Application\Log\ConsoleFormatter', $handler->getFormatter(),
    ...
    public function testWritingAndFormatting()
    {
        ...
        $output
            ->expects($this->once())
            ->method('write')
            ->with('<info>[16:21:54] app.INFO:</info> My info message'."\n")
        ;
        ...
        $errorOutput
            ->expects($this->once())
            ->method('write')
            ->with('<error>[16:21:54] app.ERROR:</error> My error message'."\n")
        ;
